		...
		int32_t exec_function() {
			...
			// Returns an archive record at 'index'
 			if(ipc.function == IPCFunctions::ARCHIVE_READ) {
				if(ipc.args[0].tag == IPCArgTypes::IPC_UINT) { //Taking in record index
					if(ipc.args[0].u.uint < ((ARCHIVE_SZ*2) / sizeof(Record))) { 
						RecordList *r = recordListHead;

						if(r == NULL) { //Failed to create a recordListhead
						...
						if(updated_archive == true) { //Happens after write
							purgeStoredRecords();
							parseArchiveRecords();
						}

						while(true) { //Hunts down record with index ipc.args[0].u.uint, weird issue, never checks if the next index is NULL so it will just keep going until it finds a value match
						...
						if(r != NULL) {
							...
							si->setIPCArgBuf(0, (uint8_t *) r, sizeof(Record)); //si wasn't deleted 
							write_ipc_handle(si->ipc);
						} else {
							SerializedIPCHelper *si = createIPCResponseERR();
							write_ipc_handle(si->ipc);
			...
			// Appends a new Record to archive.log
				...
				if(ipc.args[0].tag == IPCArgTypes::IPC_BUF) {
					Record *r = (Record *) ipc.args[0].u.buf; //Creates Record from buffer 
		...
		int32_t exec_function() {
			...
			if(ipc.function == IPCFunctions::DESKTOP_CLIPBOARD_SET) {
				...
				if(ipc.args[0].tag == IPCArgTypes::IPC_BUF && ipc.args[1].tag == IPCArgTypes::IPC_UINT) {
					if(!OpenClipboard(NULL)) { //Failed to open clipboard
						return ERR;
					}
					...
					memcpy(p, ipc.args[0].u.buf, ipc.args[1].u.uint); //A weird memcpy, definite overflow

					BitMap *bmm = (BitMap *) p; //Controlled BitMap Struct?
#ifndef SHARED_MEMORY
					// Ensure the validateBitMap function pointer is used
					bmm->f = reinterpret_cast<BitMapCb>(&validateBitMap); //b->height * b->width > UINT_MAX returns error, can this be bypassed?
#endif
					if((bmm->f(bmm)) == ERR) {
						SerializedIPCHelper *si = createIPCResponseERR();
                        write_ipc_handle(si->ipc);
			...
			// Gets a clipboard object
				...
				si->setIPCRaw((uint8_t *) b, strlen(b)); //Overflow if the Clipboard String is greater than 32768
				write_ipc_handle(si->ipc);
				delete si;
#else
				SerializedIPCHelper *si = createIPCResponseERR();
		...
		int32_t exec_function() {
			...
			if(ipc.function == IPCFunctions::FILEIO_READ) {
				if(ipc.args[0].tag == IPCArgTypes::IPC_BUF && ipc.args[1].tag == IPCArgTypes::IPC_UINT &&
					...
					ipc.args[0].u.buf[sizeof(ipc.args[0].u.buf)] = '\0'; //One off Null Pointer, Possible type change on tag, but is it out of context at that point?

					char *f = (char *) ipc.args[0].u.buf; //Put a string where the directory traversal starts past 512
					uint32_t offset = ipc.args[1].u.uint;
					uint32_t size = ipc.args[2].u.uint; 

					char *p = isValidPath(f); //checks for .. and \\, but only 512 of the first characters....
					...
					if(offset + 1 == 0 || size + 1 == 0) { //offset overflow check?
						SerializedIPCHelper *si = createIPCResponseERR();
						write_ipc_handle(si->ipc);
						delete si;
						return ERR;
					...
					SerializedIPCHelper *si = createIPCResponseOKRaw();
					si->setIPCRaw(ipcBH->retrievePtrToOffset(), size); //Still uses the offset, but doesn't subtract from the size the offset, read primitive, but it requires something next to the heap block 

					//delete ipcBH; //Early Free

					write_ipc_handle(si->ipc);
					delete si;

					char tmp[8192];
					snprintf(tmp, sizeof(tmp)-1, "[+] FILEIO_READ() - offset=%x size=%x filename=%s file=%s", offset, size, f, (char *) ipcBH->getRawPtr()); //ipcBH was was freed line 457, could also be overflow
					logStr(tmp);
					free(file); delete ipcBH; //Addition fix, bug
				}
			// FILEIO_WRITE(file, buf, offset, size)
				...
				if(ipc.args[0].tag == IPCArgTypes::IPC_BUF && ipc.args[1].tag == IPCArgTypes::IPC_BUF &&
					...
					if(size > IPC_ARGS_BUF_SZ) { //Size gets constrained to 512
						size = IPC_ARGS_BUF_SZ;
					}

					ipcBH = new BufferHelper((uint8_t *) b, size); //This guy is never deleted

					if(offset > IPC_ARGS_BUF_SZ) { //offset can't be larger than 512
						ipcBH->setOffset(0);
					} else {
						ipcBH->setOffset(offset);
					}
					...
					if(of.is_open()) {
    			        ...
    			        of.write((char *) ipcBH->retrievePtrToOffset(), ipcBH->getSize()); //the fuck happens here
						of.close();
					} else {
						SerializedIPCHelper *si = createIPCResponseERR();
						write_ipc_handle(si->ipc);
		...
		int32_t exec_function() {
			...
			if(ipc.function == IPCFunctions::CRYPTO_SET_IV) {
				if(ipc.args[0].tag == IPCArgTypes::IPC_ULONG) {
					iv = ipc.args[0].u.ulong; //sets IV via input
			...
			// CRYPTO_SET_KEY(key, key_size)
				...
				if(ipc.args[0].tag == IPCArgTypes::IPC_RAW && ipc.args[1].tag == IPCArgTypes::IPC_UINT) {
					...
					if(key_size > sizeof(ipc.raw)) { //keysize is user controlled, sizeof(ipc.raw) is closed 32768, keysize can be 32767 at max
						SerializedIPCHelper *si = createIPCResponseERR();
						write_ipc_handle(si->ipc);
						delete si;

						if(key != NULL)
							delete [] key; //Not sure honestly
					...
					memcpy(key, k, key_size); // memcpy(key, ipc.raw, range(1,32767)), huge overflow, overflow into key_size, Stack data can be written to the key, 16340+"BBBB" ~> eip control SEH overflow
					key_is_set = true;
			...
			// CRYPTO_GET_KEY()
				...
				if(key_is_set) { //if(key_is_set = false) { //BUG, key_is_set becomes false when ran, bottom loop never runs
					SerializedIPCHelper *si = createIPCResponseOKRaw();
			...
			// CRYPTO_SET_KEY_NAME(name, size)
				...
				if(ipc.args[0].tag == IPCArgTypes::IPC_BUF && ipc.args[1].tag == IPCArgTypes::IPC_USHORT) {
					...
					if((checkKeyName(kn, ksz)) == OK) {
						...
						memcpy(key_name, kn, ksz); //Overflow, maybe not, checks above and fixes it to 512
						SerializedIPCHelper *si = createIPCResponseOK();
						write_ipc_handle(si->ipc);
						delete si;
					} else {
		...
		int32_t exec_function() {
			...
			if(ipc.function == IPCFunctions::VALIDATE_BLOB) {
				if(ipc.args[0].tag == IPCArgTypes::IPC_ULONG && ipc.args[1].tag == IPCArgTypes::IPC_BUF) {
					...
					if(index < 0) { //index can't be negative, can be giant as fuck though
						SerializedIPCHelper *si = createIPCResponseERR();
						write_ipc_handle(si->ipc);
						delete si;
						return ERR;
					...
					ValidateCb vcb = reinterpret_cast<ValidateCb>(&v_array[index]); //The fuck
					vcb(b); // call [block + ([block + A0A0] + 2824) *4], need to know where block starts 
				}
			}
...
class IPC_Ping : public IPC {
    public:
		void init() {
			SerializedIPCHelper *si = createNewIPC(); //SerializedIPCHelper(0, 0, (uint32_t) this, 0) , the fuck is the (uin32_t) this //Weird Heap leak 
...
class IPC_Encode : public IPC {
	public:
		void init(SerializedIPC *d) {
			...
			if (!m_encoder || ipc.function == IPCFunctions::ENCODE_ADD_DATA) //Every time I add data m_encoder has new ran on it, mem leak?
		...
		uint32_t exec_function() {
			if (ipc.function == IPCFunctions::ENCODE_ADD_DATA) {
				if (ipc.args[0].tag == IPCArgTypes::IPC_UINT &&
					...
					m_encoder->addData(ipc.args[1].u.buf, ipc.args[0].u.uint); //data, length
				} else {
					SerializedIPCHelper *si = createIPCResponseERR();
					write_ipc_handle(si->ipc);
					delete si;
				...
				if (m_encoder)
					delete m_encoder; //The only time m_encoder can be re-init'd is by calling the init function, 
			}
		...
		class Base64Encoder : public DataEncoder {
			public:
				...
				virtual void addData(const uint8_t *data, uint32_t length) {
					uint32_t newlength = (length/3)*4; //I can overflow newlength upwards
					uint8_t *newdata = new uint8_t[m_length+newlength];

					memcpy(newdata, m_data, m_length); //(newdata, NULL, 0) ~> memcpy(newdata, m_data, m_length), 
					base64encode(data, length, newdata+m_length, newlength); 

					delete [] m_data; //m_data is a pointer not a scalar
					m_data = newdata; //sets m_data as the newly encoded data
					m_length += newlength; //sets length
...
class IPC_Auth : public IPC {
    public:
		IPC_Auth() : valid_session(false) { 
			m_password = "c5a514664b81f21235d2d1e7e6454334abc3cf4b"; //Hard coded
		}
		...
		int32_t exec_function() {
			...
			if(ipc.function == IPCFunctions::AUTH_SESSION) {
				if (ipc.args[0].tag == IPCArgTypes::IPC_UINT &&
					...
					string p((char *) ipc.args[1].u.buf, ipc.args[0].u.uint); //User controls the size data, string p((char *) ipc.args[1].u.buf, ipc.args[0].u.uint);
...
int32_t read_ipc_handle(char *filename) {
	...
	while(true) {
    	ipc_in_mem = (uint8_t *) xmalloc(sizeof(SerializedIPC) - 1);
		retval = 0;
...
#if WIN32 || WIN64
		if(shmHandle == NULL) {
			shmHandle = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_EXECUTE_READWRITE, 0, sizeof(SerializedIPC), filename); //Modified PAGE_SIZE to sizeof the SerializedIPC
		}

    	if(shmHandle == NULL) { //There's a bug right here, shmHandle can return an Error Code, but the code doesn't check for error codes whatsoever and continues
    		destroy_ipc_data(&ipc_in_mem); printf("Could not create file mapping object (%d)\n",GetLastError());
			return ERR;
    	}

		if(shmPtr == NULL) {
		    shmPtr = (LPTSTR) MapViewOfFile(shmHandle, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(SerializedIPC)); 
		}

		if(shmPtr == NULL) { printf("Could not create file mapping object (%d)\n",GetLastError());
			CloseHandle(shmHandle); 
    		destroy_ipc_data(&ipc_in_mem);
			return ERR;
    	} 

		memcpy(ipc_in_mem, shmPtr, sizeof(SerializedIPC)); //shmPtr can be a volatile value here
#endif
		...
		//0 <-> 8 
		if(is_valid_ipc_type(raw_ipc->type) == false) { //Checks ipc_in_mem->type for validity 
			cout << "Unknown type! (0x" << hex << raw_ipc->type << ")\n" << endl;
	    	destroy_ipc_data(&ipc_in_mem);
			return ERR;
		}
		//c5a514664b81f21235d2d1e7e6454334abc3cf4b
